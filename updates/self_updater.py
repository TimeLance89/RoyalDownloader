...
class SelfUpdater:
    ...
    def _extract_archive(self, archive: Path, destination: Path) -> Path:
        ...
        required_files = (
            ...
            "requirements.txt",
            "requirements.lock",
            "integrations/ui_translator.py",
            "updates/ytdlp_updater.py",
            "web/app.js",
            "web/i18n.js",
            "updates/update_checker.py",
            "updates/update_channels.py",
    ...
    def _repair_nodriver(self, app_dir: Optional[Path] = None, python: Optional[Path] = None) -> None:
        ...
        command = (
            ...
            "import core.nodriver_patch; core.nodriver_patch.ensure_cdp_utf8()"
        )
        subprocess.run(
    ...
    def _smoke_release(self, release: Path, python: Path) -> None:
        completed = subprocess.run(
            [
                ...
                "import core.config, media.downloader, media.extractor, updates.update_checker, updates.self_updater, server",
                str(release),
            ],
            cwd=str(release), capture_output=True, text=True, timeout=180, check=False,
        )
