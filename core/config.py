...
from core.queue_jobs import atomic_save as atomic_save_queue_jobs
from core.queue_jobs import load_document as load_queue_document
...
SERIES_FALLBACK_RETRY_SECONDS = _env_positive_int(
    "SERIES_FALLBACK_RETRY_SECONDS", 90,
)
SERIES_RESOLVED_LINK_CACHE_TTL_SECONDS = _env_positive_int(
    "SERIES_RESOLVED_LINK_CACHE_TTL_SECONDS", 15 * 60,
)
SERIES_RESOLVED_LINK_CACHE_MAX_ENTRIES = _env_positive_int(
    "SERIES_RESOLVED_LINK_CACHE_MAX_ENTRIES", 2000,
)
from integrations.ui_translator import DEFAULT_UI_LANGUAGE, normalize_ui_language
...
def load_module_states() -> dict[str, bool]:
    ...
    # Preserve existing opt-in Ollama users when the Intelligence module is
    # introduced. Fresh installations keep the module disabled by manifest.
    if "royal-intelligence" not in states and values.get("ai_enabled", "").strip().casefold() in {"1", "true", "yes", "on", "ja"}:
        states["royal-intelligence"] = True
    return states
...
def load_ai() -> dict:
    """Load optional Royal Intelligence; old Ollama settings stay valid."""
    values = _read_all()
